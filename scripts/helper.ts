...
export async function createOrUpdateConfigFile(
  ...
  }: { senderAddress?: string; receiverAddress?: string, transferUsdcTokenAddress?: string }
) {
  ...
  if (fs.existsSync(deployedContracts)) {
    ...
    if (transferUsdcTokenAddress) config[network].transferUsdcTokenAddress = transferUsdcTokenAddress;
  } else {
    ...
    if (transferUsdcTokenAddress) {
      config = {
        [network]: {
          transferUsdcTokenAddress: transferUsdcTokenAddress,
        },
      };
  }
